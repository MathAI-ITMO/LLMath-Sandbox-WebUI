<template>
  <div class="llmath-problems-view">
    ...
    <div v-if="validationWarning" class="error-message">
      {{ validationWarning }}
    </div>
    <div v-if="llmSolutionError" class="error-message">
      {{ llmSolutionError }}
    </div>
    <div v-if="checkSolutionError" class="error-message">
      {{ checkSolutionError }}
    </div>
    ...
    <div v-if="activeTab === 'management'" class="tab-content management-tab">
      ...
      <section v-if="!editingProblem" class="api-section form-section geolin-load-section">
        ...
        <div v-if="geoSeedMessage" class="info-message">
          {{ geoSeedMessage }}
        </div>
        <button @click="loadFromGeolin" :disabled="apiCallLoading.loadFromGeolin">Загрузить данные из GeoLin</button>
        <div v-if="apiResponse.loadFromGeolin" class="api-response">
          <strong>Ответ:</strong> <pre>{{ JSON.stringify(apiResponse.loadFromGeolin, null, 2) }}</pre>
        </div>
      </section>
...
import { useTestLLMathProblems } from '@/composables/useTestLLMathProblems';
...
const {
  ...
  apiState,
  problemIdToFetch,
} = useTestLLMathProblems();

const problems = problemsCatalog.list;
const loading = problemsCatalog.loading;
const error = problemsCatalog.error;
const attemptedLoad = problemsCatalog.attemptedLoad;
const foundProblemsByTypeList = problemsCatalog.foundByType;
const foundProblemByIdList = problemsCatalog.foundById;
const typeToFetchProblemsBy = problemsCatalog.typeQuery;
const allTypes = problemsCatalog.types;
const fetchAllProblems = problemsCatalog.fetchAll;
const fetchProblemById = problemsCatalog.fetchById;
const fetchProblemsByType = problemsCatalog.fetchByType;
const fetchAllTypes = problemsCatalog.fetchTypes;
const getProblemAssignedTypes = problemsCatalog.getProblemAssignedTypes;

const newProblem = managementForms.newProblem.data;
const newProblemSolutionStepsJson = managementForms.newProblem.solutionJson;
const newProblemLlmSolutionJson = managementForms.newProblem.llmSolutionJson;
const createProblem = managementForms.newProblem.create;

const managementNewProblem = managementForms.management.data;
const managementNewProblemSolutionStepsJson = managementForms.management.solutionJson;
const managementNewProblemLlmSolutionJson = managementForms.management.llmSolutionJson;
const managementNewProblemType = managementForms.management.type;
const addProblemFromManagementTab = managementForms.management.add;

const editingProblem = managementForms.editing.record;
const currentEditProblem = managementForms.editing.current;
const currentEditProblemType = managementForms.editing.type;
const currentEditProblemSolutionStepsJson = managementForms.editing.solutionJson;
const currentEditProblemLlmSolutionJson = managementForms.editing.llmSolutionJson;
const showEditForm = managementForms.editing.show;
const cancelEdit = managementForms.editing.cancel;
const updateProblemFromManagementTab = managementForms.editing.update;

const updateProblemData = managementForms.update.data;
const updateProblemSolutionStepsJson = managementForms.update.solutionJson;
const updateProblemLlmSolutionJson = managementForms.update.llmSolutionJson;
const setProblemToUpdate = managementForms.update.setFromProblem;
const loadProblemForUpdate = managementForms.update.load;
const updateProblem = managementForms.update.update;

const problemIdToDeleteValue = managementForms.removal.id;
const setProblemToDelete = managementForms.removal.set;
const deleteProblemFromDbTab = managementForms.removal.deleteFromDb;
const deleteProblemByIdAndRefresh = managementForms.removal.deleteFromList;

const typeAssignment = managementForms.typeAssignment.form;
const assignTypeToProblem = managementForms.typeAssignment.assign;

const geolinPrefixToLoad = geoLinTools.prefix;
const availableGeolinPrefixes = geoLinTools.availablePrefixes;
const geoSeedMessage = geoLinTools.message;
const loadFromGeolin = geoLinTools.load;

const activeForm = solutionActions.activeForm;
const validationWarning = solutionActions.validationWarning;
const llmSolutionError = solutionActions.llmSolutionError;
const checkSolutionError = solutionActions.checkSolutionError;
const getLlmSolution = solutionActions.getLlmSolution;
const checkSolution = solutionActions.checkSolution;
const checkResultModal = solutionActions.modal;
const closeCheckResultModal = solutionActions.closeModal;

const selectedProblem = modalState.selectedProblem;
const showProblemDetails = modalState.showProblemDetails;
const closeModal = modalState.closeModal;

const apiCallLoading = apiState.apiCallLoading;
const apiResponse = apiState.apiResponse;
</script>
