...
// removed duplicate vue import; renderMessage not used here
...
// Remove codai links from the rendered messages (do not drop entire lines)
// This is kept for backward compatibility with old messages containing codai.ru links
const messagesForRender = computed(() => {
  if (!hasTheory.value) return messages.value
  return (messages.value ?? []).map(m => {
    if (!m?.text) return m
    const newText = m.text
      // strip all codai links in-line, keep the rest of the text
      .replace(codaiReAll, '')
      // remove blank lines left after removal
      .replace(/^\s*$/gm, '')
    if (newText === m.text) return m
